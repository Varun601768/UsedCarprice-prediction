...
def send_email():
    try:
        # Get form data (assuming FormData, not JSON)
...
# Optional: Add CSRF token endpoint if needed
@app.route('/get-csrf-token', methods=['GET'])
def get_csrf_token():
    return jsonify({'csrf_token': 'dummy-token'})  # Replace with actual CSRF logic if needed
...
@app.route('/api/UsedCar', methods=['POST'])
def seed_database():
